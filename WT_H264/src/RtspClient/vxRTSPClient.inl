...
//mjpeg图像数据
#define RTSP_FRAMETYPE_REALMJPEG	103
...
void vxRtspCliHandle::Stream_OnEvent_RecvedRealFrame(void * pv_handle, void * frame_buf, unsigned int frame_size, void * pv_user)
{
	...
	}else if ((0 == _stricmp(sz_media_name, "video")) && (0 == _stricmp(sz_codec_name, "JPEG")))
	{
		if (NULL != obj_ptr->m_func_recved)
		{
			obj_ptr->m_func_recved(obj_ptr, frame_buf, frame_size, RTSP_FRAMETYPE_REALMJPEG, obj_ptr->m_pv_user);
		}
	}
}
